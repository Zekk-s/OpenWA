...
Dev modu (hot reload, API + dashboard separate servers):

```bash
npm run dev
```

Bu komut iki server başlatır:
- **API:** `http://localhost:2785`
- **Dashboard (Vite):** `http://localhost:2886`

Alternatif olarak API'yi ayrı başlat:
...
Tarayıcı açıp git:
- **Dev modu:** `http://localhost:2886` (Vite dev server)
- **Prod/Docker:** `http://localhost:2785` (API ile aynı origin)
...
OpenWA yalnızca MCP server endpoint'ini (`http://localhost:2785/mcp`) expose eder. Slack entegrasyonu harici bir bot/app tarafından gerçekleştirilir:
- `.env`'de `MCP_ENABLED=true` ayarla
- Slack bot/app (ör. Claude Code Slack connector) OpenWA MCP endpoint'ine bağlanır
- Slack DM'ye WhatsApp komutları gönder (bot MCP aracılığıyla çeviri yapar)

Detaylar için: `README.md` → MCP section
...
Prod build için: `docker compose up -d` (detaylar için `docker-compose.yml` dosyasını incele)
